...
- All colors flow through CSS custom properties `--c-*` (space-separated RGB channel triplets), defined in `src/index.css`. `:root` holds MIDNIGHT's tokens as the cascade fallback (the app defaults to `matcha`); a `[data-theme="…"]` block per `THEMES` entry overrides them.
