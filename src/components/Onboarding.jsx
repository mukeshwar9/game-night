...
// { gameId, gameType?, hostName?, hostAvatar? }), where it ends in JOIN GAME.
...
function InviteCard({ invite, cfg }) {
  ...
  return (
    <div className="bg-retro-card border-2 border-retro-cta/60 rounded p-4 space-y-3 text-center">
      <p className="font-pixel text-sm text-retro-cta text-glow-cta tracking-wider">YOU&apos;RE INVITED!</p>
      {cfg && (
      ...
      {invite.hostName && (
        <div className="flex items-center justify-center gap-2">
          ...
          <span className="font-mono text-xs text-retro-text truncate"><span className="text-retro-dim">with </span>{invite.hostName}</span>
        </div>
      )}
