...
describe('commit-reveal round flow', () => {
  it('commitments hide answers until reveal; only verified reveals are tallied', async () => {
    ...
    expect(pendingReveals(answers, reveals)).toEqual([])
    const { tally, cheats } = await verifyHerdReveals(answers, reveals, verifyReveal)
    expect(tally).toEqual({ a: 'Pepperoni' })
    expect(cheats).toEqual({ b: true })
  })
...
describe('armAnswerDeadline — server-time answer clock', () => {
  ...
  it('arms an unset clock once: startedAt = server now, endsAt = now + the window', () => {
    expect(armAnswerDeadline(fresh, 3, 1000)).toEqual({ ...fresh, startedAt: 1000, endsAt: 1000 + ANSWER_MS })
    ...
    // The armed round's live deadline matches what was stamped.
    expect(answerDeadline(armAnswerDeadline(fresh, 3, 1000), 1)).toBe(1000 + ANSWER_MS)
  })

  it('takes the room-scaled window, and stamps only startedAt when timers are off', () => {
    expect(armAnswerDeadline(fresh, 3, 1000, 2 * ANSWER_MS).endsAt).toBe(1000 + 2 * ANSWER_MS)
    const off = armAnswerDeadline(fresh, 3, 1000, null)
    expect(off.startedAt).toBe(1000)
    expect(off.endsAt).toBeNull()
  })

  it('never overwrites a clock that is already set (old rounds included)', () => {
    expect(armAnswerDeadline({ ...fresh, endsAt: 5 }, 3, 1000)).toBeUndefined()
    expect(armAnswerDeadline({ ...fresh, startedAt: 5 }, 3, 1000)).toBeUndefined()
  })
