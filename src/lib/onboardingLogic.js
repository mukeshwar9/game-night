...
import { NAME_REJECT_MESSAGES, sanitizeDisplayName } from './moderationLogic'
...
export function validateName(raw) {
  ...
  // Same moderation as every other name entry (moderationLogic.js).
  const moderated = sanitizeDisplayName(name)
  if (!moderated.name) return { ok: false, name, error: NAME_REJECT_MESSAGES[moderated.reason] }
  return { ok: true, name: moderated.name, error: null }
}
